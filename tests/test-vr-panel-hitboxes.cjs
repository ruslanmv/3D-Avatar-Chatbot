...
function testHitboxModeTags() {
    ...
    log(
        code.includes("mesh.userData.mode = 'global'") ? 'PASS' : 'FAIL',
    ...
    log(
        !hitboxFn.includes("handle.userData.mode = 'chat'") ? 'PASS' : 'FAIL',
...
function testGetActiveInteractables() {
    ...
    log(code.includes('getActiveInteractables()') ? 'PASS' : 'FAIL', 'getActiveInteractables() method exists');

    // Must filter by mode tag
    log(
        code.includes("mTag === 'global' || mTag === mode") ? 'PASS' : 'FAIL',
...
function testModeChangeCallback() {
    ...
    const setModeBlock = code.substring(code.indexOf('setMode(mode)'), code.indexOf('setStatus(status)'));
    log(
        setModeBlock.includes('this.onModeChanged') ? 'PASS' : 'FAIL',
...
function testHandleUIActionModeGuard() {
    ...
    log(
        fnBlock.includes('hitMode') && fnBlock.includes('this.mode') ? 'PASS' : 'FAIL',
        'handleUIAction checks hitbox mode against current panel mode'
    );

    log(
        fnBlock.includes("hitMode !== 'global'") ? 'PASS' : 'FAIL',
        'handleUIAction allows global hitboxes through mode guard'
    );

    log(
        fnBlock.includes('return false') ? 'PASS' : 'FAIL',
...
function testActiveUIInteractables() {
    ...
    log(code.includes('_activeUIInteractables') ? 'PASS' : 'FAIL', '_activeUIInteractables field exists');

    log(code.includes('updateActiveUI(') ? 'PASS' : 'FAIL', 'updateActiveUI() method exists');
    ...
    log(
        triggerBlock.includes('this._activeUIInteractables') ? 'PASS' : 'FAIL',
    ...
    log(
        hoverBlock.includes('this._activeUIInteractables') ? 'PASS' : 'FAIL',
    ...
    log(
        gripBlock.includes('this._activeUIInteractables') ? 'PASS' : 'FAIL',
    ...
    log(
        regBlock.includes('this._activeUIInteractables = interactables') ? 'PASS' : 'FAIL',
...
function testNoStaleRaycasts() {
    ...
    log(
        raycastCalls.length === 0 ? 'PASS' : 'FAIL',
...
function testIntegrationWiring() {
    ...
    log(code.includes('updateActiveUI') ? 'PASS' : 'FAIL', 'VRChatIntegration calls updateActiveUI');

    // Must set onModeChanged callback
    log(code.includes('onModeChanged') ? 'PASS' : 'FAIL', 'VRChatIntegration hooks onModeChanged callback');

    // Must call getActiveInteractables
    log(
        code.includes('getActiveInteractables()') ? 'PASS' : 'FAIL',
