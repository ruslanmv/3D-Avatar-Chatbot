...
describe('the assets exist, and the catalogue names them', () => {
    ...
    test('they are WebP, and none is large enough to stall a settings panel', () => {
        // A19: both plates now, not only the wide one. The portrait file is downloaded on exactly
        // the devices least able to afford a surprise, so it is held to the same figure.
        for (const scene of manifest.scenes) {
            for (const field of ['src', 'srcPortrait']) {
                if (!scene[field]) continue;
                const file = path.join(root, scene[field]);
                expect(scene[field].endsWith('.webp')).toBe(true);
                ...
                expect(fs.statSync(file).size).toBeLessThan(450 * 1024);
            }
        }
    });

    test('choosing a scene costs one plate, and that is what the budget is about', () => {
        /**
         * The number a visitor actually pays. A19 replaced ten 1672x941 photographs (1.25 MB in
         * total) with twenty full-resolution plates composed for this app's camera, so the old
         * whole-set figure no longer describes anything a person experiences: nobody downloads
         * the set. They pick a scene and fetch one picture, and `sourceFor` guarantees it is one
         * and not both.
         *
         * So the budget moved to where the cost is. 450 KB is roughly a second on a slow
         * connection, and the largest plate here is 394 KB.
         */
        const plates = manifest.scenes.flatMap((s) => [s.src, s.srcPortrait].filter(Boolean));
        const largest = Math.max(...plates.map((f) => fs.statSync(path.join(root, f)).size));
        expect(largest).toBeLessThan(450 * 1024);
    });

    test('and the repository pays for all of them once', () => {
        // Still worth a cap: this is a static site, and the whole set is in every clone and every
        // deploy. Twenty plates at ~300 KB is 6 MB; 8 leaves room without leaving the door open.
        const plates = manifest.scenes.flatMap((s) => [s.src, s.srcPortrait].filter(Boolean));
        const total = plates.reduce((n, f) => n + fs.statSync(path.join(root, f)).size, 0);
