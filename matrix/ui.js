(function () {
  ...
  function startAvatarViz() {
    ...
    function setState(v) { state.value = v; }
    ...
    function draw() {
      ...
      // main ring
      const pulse =
        state.value === 'thinking' ? 1 + Math.sin(t * 6) * 0.03 :
        state.value === 'listening' ? 1 + Math.sin(t * 10) * 0.05 :
        state.value === 'speaking' ? 1 + Math.sin(t * 12) * 0.04 :
        state.value === 'error' ? 1 + Math.sin(t * 16) * 0.02 :
        1 + Math.sin(t * 2) * 0.01;
      ...
      const eyeX = w * 0.06;
      let eyeGlow =
        state.value === 'listening' ? 0.9 :
        state.value === 'thinking' ? 0.7 :
        state.value === 'speaking' ? 1.0 :
        state.value === 'error' ? 0.25 :
        0.55;

      ctx.fillStyle = `rgba(0,255,123,${0.15 + eyeGlow * 0.25})`;
      ctx.beginPath(); ctx.arc(cx - eyeX, eyeY, w * 0.02, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(cx + eyeX, eyeY, w * 0.02, 0, Math.PI * 2); ctx.fill();
      ...
      const y = cy + w * 0.15;
      const amp =
        state.value === 'speaking' ? w * 0.045 :
        state.value === 'thinking' ? w * 0.028 :
        state.value === 'listening' ? w * 0.020 :
        state.value === 'error' ? w * 0.010 :
        w * 0.012;
  ...
  function closeSettings() {
    try { els.settingsModal.close(); } catch {}
  ...
  async function main() {
    ...
    if (els.closeSettings) els.closeSettings.addEventListener('click', (e) => { e.preventDefault(); closeSettings(); });
