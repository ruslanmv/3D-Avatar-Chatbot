 ...
 * - Pollinations for AI-generated images (deployment-side key only)
 * - one Discovery & Media preference: "Image source"
...
(function (global) {
    ...
    const PexelsProvider = {
        ...
        async search(query, options = {}) {
            ...
            // A key the user deliberately entered wins over the deployment's quota, matching
            // the repository's existing web-search ownership rule.
    ...
    async function chooseProvider(mode, options = {}) {
        ...
        if (mode === 'ai') {
            const provider = namedProvider('pollinations');
    ...
    function unavailableSentence(mode) {
        if (mode === 'ai') {
            return 'AI image generation is not configured. Set POLLINATIONS_API_KEY on the deployment, then choose Pollinations under Settings → Discovery & Media.';
        }
        if (mode === 'real') {
        ...
        return 'No image provider is ready. Configure Pexels or Pollinations in Settings → Discovery & Media.';
    }
    ...
    async function handleIntent(intent, originalText, doc, options = {}) {
        ...
        const max = provider.ID === 'pollinations' ? 1 : safeCount(options.max, 4, 8);
        let results = [];
        try {
        ...
        const summary =
            provider.ID === 'pollinations'
                ? 'Here is an AI-generated image from Pollinations.ai.'
                : `Here ${results.length === 1 ? 'is' : 'are'} ${results.length} photo${results.length === 1 ? '' : 's'} from Pexels.`;
    ...
    function installSettingsPanel(doc) {
        ...
        note.textContent =
            'Image source is selected above. A personal Pexels key is sent only through this app’s same-origin proxy. Pollinations uses POLLINATIONS_API_KEY on the server; secret AI keys are never stored in the browser.';
