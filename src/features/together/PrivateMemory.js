 ...
 * said, the escalation level reached, the scene, or any free text whatsoever. The whole record
 * is four enums and two numbers, and `write` refuses anything that is not one of them — so a
 * later caller cannot quietly start storing content through a door this file left open. That
 * is the difference between "we do not store the conversation" and "we cannot".
 *
...
(function (global) {
    ...
    const EMPTY = Object.freeze({ preset: null, mood: null, soundtrack: null, sessions: 0, lastAt: 0 });
    ...
    function write(patch) {
        const next = sanitise({ ...read(), ...(patch && typeof patch === 'object' ? patch : {}) });
        const s = store();
        if (!s) return next;
        try {
    ...
    function remember({ preset = null, mood = null, soundtrack = null } = {}) {
        const current = read();
        return write({
    ...
    const api = { KEY, FIELDS, COUNTERS, EMPTY, read, write, remember, preferredPreset, isFirstSession, forget };
