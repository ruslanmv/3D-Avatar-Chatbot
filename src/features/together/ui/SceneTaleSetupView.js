 ...
 *
 * The activity owns state and behavior. This module only turns the existing configure DOM
 * into HomePilot's cinematic setup surface: current-scene preview, idea field, soundtrack
 * cards and one primary Create story action. Existing inputs/buttons are moved, not replaced,
 * so their listeners and accessibility behavior remain intact.
 *
 * Exposes: window.NEXUS_SCENE_TALE_SETUP_VIEW
 */
...
#${PANEL_ID}.${PANEL_CLASS}{
  ...
  background:
    radial-gradient(120% 90% at 50% -10%,rgba(25,214,244,.095),transparent 50%),
    linear-gradient(180deg,rgba(8,20,31,.965),rgba(5,14,23,.95));
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-head{
  margin:0 auto .55rem; display:flex; align-items:center; justify-content:center; gap:1.15rem;
  color:#22e8ff; font-size:.76rem; line-height:1; font-weight:760; letter-spacing:.24em; text-align:center;
}
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-head::before,
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-head::after{content:'';display:block;width:min(7.5rem,24%);height:1px;background:linear-gradient(90deg,transparent,rgba(34,232,255,.72))}
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-head::after{transform:scaleX(-1)}
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-subtitle.nexus-scene-tale-setup-title{
  margin:.1rem 0 .28rem; color:#f5fbff; font-size:clamp(2rem,4.1vw,2.7rem); line-height:1.04; font-weight:760; letter-spacing:.015em; text-align:center;
}
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-prompt{
  margin:0 0 1.65rem; color:#92aecd; font-size:1rem; line-height:1.4; text-align:center;
}
#${PANEL_ID}.${PANEL_CLASS} .nexus-scene-tale-setup-section{display:block;margin:0 0 1.25rem}
#${PANEL_ID}.${PANEL_CLASS} .nexus-story-setup-label.nexus-scene-tale-section-label{
  display:flex; align-items:center; gap:.72rem; margin:0 0 .58rem; opacity:1; color:#dff5ff;
  font-size:.95rem; line-height:1.2; font-weight:650; letter-spacing:.005em;
}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-scene-tale-place-card{
  margin:0; min-height:4.55rem; display:flex; align-items:center; gap:1rem; padding:.62rem .78rem;
  border-radius:14px; border:1px solid rgba(121,174,214,.34); background:linear-gradient(180deg,rgba(25,43,60,.66),rgba(13,29,44,.66));
  box-shadow:inset 0 1px 0 rgba(255,255,255,.025); color:#deeffc; font-size:1.02rem; font-weight:520;
}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-story-setup-input{
  width:100%; min-height:4.15rem; resize:none; overflow:hidden; padding:1rem 1.1rem; border-radius:14px;
  border:1px solid rgba(25,223,249,.88); outline:none; background:linear-gradient(180deg,rgba(20,39,55,.74),rgba(10,26,40,.74));
  box-shadow:0 0 0 1px rgba(16,217,245,.06),inset 0 1px 0 rgba(255,255,255,.025); color:#eaf6ff;
  font:500 1rem/1.35 var(--font-sans,Inter,system-ui,sans-serif); caret-color:#20e4fb;
}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-story-radio.nexus-scene-tale-sound-option{
  position:relative; display:flex; align-items:center; gap:.8rem; min-height:4rem; margin:0; padding:.85rem 1rem;
  border:1px solid rgba(120,164,201,.35); border-radius:14px; background:linear-gradient(180deg,rgba(20,39,55,.58),rgba(10,27,41,.58));
  color:#dcecf7; font-size:.94rem; font-weight:520; cursor:pointer; transition:border-color .15s ease,background .15s ease,box-shadow .15s ease,transform .15s ease;
}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-scene-tale-radio-dot{width:1.05rem;height:1.05rem;flex:0 0 1.05rem;border-radius:50%;border:2px solid #57718a;box-shadow:inset 0 0 0 4px rgba(9,29,43,.95);transition:border-color .15s ease,background .15s ease,box-shadow .15s ease}
...
#${PANEL_ID}.${PANEL_CLASS} [data-action='create-story']{
  position:relative; width:100%; min-height:4.35rem; display:flex; align-items:center; justify-content:center; padding:1rem 4rem;
  border:1px solid rgba(69,241,255,.92); border-radius:999px; background:linear-gradient(102deg,#18bde9 0%,#31e3e5 50%,#19bfe8 100%);
  box-shadow:0 12px 32px rgba(0,176,220,.2),inset 0 1px 0 rgba(255,255,255,.34); color:#03121b;
  font:760 1.02rem/1 var(--font-sans,Inter,system-ui,sans-serif); letter-spacing:.005em; text-align:center; cursor:pointer;
  transition:transform .15s ease,filter .15s ease,box-shadow .15s ease;
}
#${PANEL_ID}.${PANEL_CLASS} [data-action='create-story']:hover{filter:brightness(1.06);box-shadow:0 14px 36px rgba(0,196,231,.27),inset 0 1px 0 rgba(255,255,255,.38)}
#${PANEL_ID}.${PANEL_CLASS} [data-action='create-story']:active{transform:translateY(1px)}
#${PANEL_ID}.${PANEL_CLASS} [data-action='create-story']:focus-visible{outline:2px solid #b8f8ff;outline-offset:3px}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-cancel{
  position:relative; margin:1.4rem auto 0; padding:.55rem 2rem; width:auto; min-width:8.5rem; color:#7f96ad; font-size:.88rem; letter-spacing:.01em;
}
...
#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-cancel:hover{color:#cfe8f6}
@media(max-width:760px){
  #${PANEL_ID}.${PANEL_CLASS}{position:fixed;top:auto;bottom:0;left:0;right:0;transform:none;width:auto;max-height:min(90dvh,48rem);padding:1.35rem 1.15rem max(1rem,env(safe-area-inset-bottom));border-radius:22px 22px 0 0}
  #${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-subtitle.nexus-scene-tale-setup-title{font-size:2rem}
  #${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-prompt{margin-bottom:1.25rem;font-size:.92rem}
  #${PANEL_ID}.${PANEL_CLASS} .nexus-scene-tale-place-thumb{width:5.4rem;flex-basis:5.4rem;height:3.1rem}
  #${PANEL_ID}.${PANEL_CLASS} .nexus-scene-tale-sound-grid{grid-template-columns:1fr}
  #${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-cancel::before,#${PANEL_ID}.${PANEL_CLASS} .nexus-bd-together-cancel::after{width:3.5rem}
}
`;
    ...
    function svgIcon(doc, kind, className) {
        ...
        if (kind === 'pin') {
            ...
            circle.setAttribute('cx', '12'); circle.setAttribute('cy', '10'); circle.setAttribute('r', '2');
    ...
    function sceneThumbnail(label) {
        return SCENE_THUMBNAILS[String(label || '').trim()] || '';
    }
    ...
    function clearStage(panel) {
        ...
        if (stage) stage.classList.remove(OPEN_CLASS);
    ...
    function decorate(doc) {
        ...
        const stage = panel.closest && panel.closest('.avatar-card');
        panel.classList.add(PANEL_CLASS);
        if (stage) stage.classList.add(OPEN_CLASS);
        activePanel = panel;
        ...
        if (place) {
            const label = String(place.textContent || '').trim();
            place.textContent = '';
            ...
            name.textContent = label || 'Current place';
        ...
        const arrow = svgIcon(doc, 'arrow', 'nexus-scene-tale-cta-arrow');
