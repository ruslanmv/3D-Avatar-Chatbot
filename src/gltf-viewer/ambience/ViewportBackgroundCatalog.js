...
(function (global) {
    ...
    function normalizeImage(raw) {
        ...
        // A19. The optional second composition, for a portrait viewport. Absent is the normal
    ...
    /**
     * Which of an entry's pictures belongs in a viewport of this shape.
     *
     * A plate is composed for one projection. Cropping the landscape one to fill a phone held
     * upright moves the horizon — a 16:9 image cropped to 9:16 keeps a third of its width and
     * none of its calibration — and the horizon is the entire contract between the art and the
     * camera. So when a scene ships a portrait composition, a portrait viewport gets it.
     *
     * Falls back to `src` for every entry that has no second picture, which is all of them until
     * the Studio publishes a two-plate package. That fallback is the old behaviour exactly: one
     * image, cover-cropped, slightly wrong on a phone and much better than nothing.
     */
