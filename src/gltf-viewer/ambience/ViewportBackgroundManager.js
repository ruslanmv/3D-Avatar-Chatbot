...
(function (global) {
    ...
    class ViewportBackgroundManager {
        ...
        async apply(id, options) {
            ...
            // A19. Which of the entry's pictures suits the viewport we are in. One image per
            // entry until a Studio package brings two, and then the phone gets the one composed
            // for a phone rather than the wide one with two thirds of its width cropped away.
            const src = this._sourceFor(entry);

            let texture = null;
            try {
                this.loads += 1;
                texture = await this._loadTexture(src, entry);
            } catch (error) {
        ...
        onResize(w, h) {
            ...
            // A19. Rotating a phone does not change the crop, it changes which picture is right.
            // Only for an entry that actually ships two — and only when the wanted source is not
            // the loaded one, so the common case stays what it was: two vectors written, no
            // allocation, cheap enough for every debounced resize.
            const want = this._sourceFor(this._entry, w, h);
            if (want && want !== this._loadedSrc) {
                // Fire and forget: the caller is a resize handler and cannot await. `apply`
                // carries the generation counter, so a second rotation mid-load discards the
                // first result rather than racing it onto the screen.
                Promise.resolve(this.apply(this._entry.id, { force: true })).catch((error) => {
                    console.warn('[ViewportBackground] could not swap composition on resize', error);
                });
                return true;
            }
        ...
        /**
         * The picture this entry should be showing right now.
         *
         * Measured from the canvas rather than the window: in Companion mode and in a split view
         * they are different shapes, and the one the texture is cropped to is the canvas.
         */
        _sourceFor(entry, w, h) {
            const size = this._viewport(w, h);
            // No measurement yet — during construction, or before the canvas has a size. Treat it
            // as landscape: that is what every entry has, and the next resize corrects it.
            const aspect = size && size.h ? size.w / size.h : 1;
            const catalog = this.catalog;
            if (catalog && typeof catalog.sourceFor === 'function') return catalog.sourceFor(entry, aspect);
