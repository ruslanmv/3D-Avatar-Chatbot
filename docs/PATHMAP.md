...
`src/features/together/heuristics/RepCounter.js`, `kb/scripts/tag-exercises.mjs`,
`scripts/make-pose-fixture.mjs`, `tests/fixtures/pose/squat-set.json`. Touched:
`EventBus.js` (three events), `boot.js`, `UtilityRanker.js` (one argument),
`scene-journey.js` (one overlay field), `audit-budgets.mjs` (a row).

The heaviest activity in the pack, so almost every decision here is about giving frame time
back.

### Pose joins the loader that is already there

`FaceTracker` and `HandTracker` both load MediaPipe tasks-vision 0.10.14 and both cache the
module on `window.__MEDIAPIPE_VISION__` and the fileset on `window.__MEDIAPIPE_FILESET__`
precisely so the second one to start pays nothing. `PoseLandmarker` is the third and reads
those caches before reaching for the network — a test asserts the warm path imports **zero**
times, because a second copy of a 2 MB WASM runtime is not a performance problem, it is a bug
with a download attached. Another test reads `HandTracker.js` to confirm the version string
still matches.

Throttled to 20 fps desktop / 15 fps mobile from the same pair of intervals its siblings use.
Pose at 60 fps would be perfectly useless: a squat takes two seconds and nobody's knee moves
meaningfully in 16 ms.

### Fidgets pause through the gate that already exists

Not a new suppression flag. The coach installs a profile overlay whose `allows(clip)`
declines anything at or below priority 2, and §6.5's single enforcement point in
`UtilityRanker` does the rest. A second "do not play this now" mechanism is how two of them
end up disagreeing, and the one that loses is always the one the reviewer did not know about.

Making that work turned up **two defects in landed batches**, both mine, both from B23:

* `UtilityRanker.modeAllows` called `mode.allows(clip)` with one argument, so B23's play
  profile — which refuses a walking clip while the game has the player's attention — read
  `undefined` for the blackboard and always returned true. The rootMotion gate had never
  fired. Fixed by passing the blackboard as a second argument; profiles that ignore it are
  unaffected.
* `SceneJourney.derive`'s `OVERLAY_FIELDS` did not include `allows`, so *both* B23's and
  B27's overlays had their `allows` silently merged away. Fixed by adding it — and only when
  it is a **function**, so B14's rule that a scene manifest may not change what may play is
  untouched: a manifest is JSON and can never carry one. B14's test asserted the old
  behaviour and has been split into the two claims it was bundling.

### Reps: two thresholds, not one

A rep is a round trip through a band — past `low`, then past `high` — plus a refractory
period. A Schmitt trigger, and it is the whole algorithm. The obvious midpoint-crossing
counter turns a knee hovering at 90° with a degree of tracking noise into forty reps in three
seconds.

`RepCounter` never sees a landmark. `coach.js` reduces a pose to a scalar and hands it over,
which is what lets the acceptance test drive a whole set through the counter with no
MediaPipe, no WASM and no video decoder.

### The fixture is synthesised, and says so

There is no video in this repository. A real capture of somebody exercising is personal data;
a stock clip needs a licence this project does not hold. So `tests/fixtures/pose/squat-set.json`
is generated by `scripts/make-pose-fixture.mjs` and its own `provenance` field says so rather
than leaving a reader to assume otherwise.

What makes it a real test anyway is that it is not a clean sine wave. It encodes the four
shapes that actually break rep counters, each accounted for in the ground truth:

| shape | why it is there |
|---|---|
| 8 brisk reps at 2.0 s | the easy case |
| **1 partial rep to 118°** | must not count — the most valuable 80 frames in the file |
| 8 s standing pause | a midpoint counter turns this into dozens of reps |
| 4 tiring reps at 2.8 s | sets slow down; a fixed-window counter misses these |
| ±1.5° seeded noise | what a lite pose model gives you at 15 fps |

Ground truth is 12, known by construction rather than by somebody watching a video and
counting — the one thing a synthetic fixture is genuinely better at. The counter reads
**exactly 12**, and raising `low` above the partial's 118° makes the same data read 13, which
is what proves the thresholds are doing the work.

### The demo clip is chosen by intent, and refused when there is none

`demo()` emits an intent name from `RepCounter.EXERCISES` and Tier 1 picks. The file holds no
clip id and no filename — asserted by grepping it for `bvh_`, `vrma_`, `.bvh`, `.vrma`,
`.fbx` and `vendor/animations`.

That required a **KB content pass**, because it was impossible before: every exercise clip
carried only the generic `exercise` intent, so asking for jumping jacks and getting a crunch
was selection by luck. `kb/scripts/tag-exercises.mjs` adds the specific intent each clip
already depicts, working from the tags the B1/B2 harvester wrote rather than from a list of
ids — a new exercise clip picks up its intent by running it again. It only adds, never
removes or reorders, and it is idempotent. Five records changed; embeddings rebuilt.

And it **invents nothing**. There is no squat, push-up or plank asset here, so asking for a
squat demo gets "I do not have a clip for squat yet" rather than a jog. Playing the wrong
movement in the one activity where the user is copying what they see is worse than playing
nothing.

### The frame budget

`audit-budgets.mjs` gained a `coach-frame` row against §9's 2 ms, measured at **0.0009 ms**
for the engine half — the throttle check on every frame plus the landmark reduction and rep
counting on the fifteen to twenty that get through. The pose inference itself is MediaPipe's
cost on a GPU that is not in this process and stays on the checklist, where the batch's own
criterion puts it: **the budget test runs on the reference device, not a laptop.**

Like B24's row it carries a vacuity guard (`reps > 0`): a reduction that returned NaN would
cost nothing and read as the fastest code in the file. Sizing that synthetic set was itself a
bug — the first draft made every rep 640 ms, so all 750 were rejected as too fast, and the
guard is what said so.
